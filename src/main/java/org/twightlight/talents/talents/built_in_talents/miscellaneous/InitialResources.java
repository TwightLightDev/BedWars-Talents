...
public class InitialResources implements Talent<Void> {
    private final TalentsCategory category;
    private final List<Integer> costList;
    public InitialResources(TalentsCategory category, List<Integer> costList) {
    ...
    public Void handle(int level, List<Object> params) {
        ...
        int gold = 1 + Math.round(level/4) + 2 * Math.round(level/20);
        int iron = level * 3;
