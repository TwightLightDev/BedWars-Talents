...
public class DamageReduction implements Talent<Double> {
    private final TalentsCategory category;
    private final List<Integer> costList;
    public DamageReduction(TalentsCategory category, List<Integer> costList) {
        this.category = category;
        this.costList = costList;
    }

    public Double handle(int level, List<Object> params) {
        double totalReduction = level*0.0125;
        ...
        if (ConversionUtil.getPlayerFromBukkitPlayer(p) != null) {
            ...
            Double add = player.getAttribute(PlayerAttribute.TOTAL_ADDITIONAL_DAMAGE_REDUCTION);
            totalReduction += add;
