...
public class CriticalArrow implements Talent<Double> {
    private final TalentsCategory category;
    private final List<Integer> costList;
    public CriticalArrow(TalentsCategory category, List<Integer> costList) {
    ...
    public Double handle(int level, List<Object> params) {
        ...
        if (ConversionUtil.getPlayerFromBukkitPlayer(p) != null) {
            ...
            Double total_addition_chance = player.getAttribute(PlayerAttribute.ARROW_CRITICAL_CHANCE);
            chance += total_addition_chance;
            DebugService.debugMsg("Attribute modified, new value: " + chance);
            Double total_addition = player.getAttribute(PlayerAttribute.ARROW_CRITICAL_DAMAGE);
        ...
        DebugService.debugMsg("Rolling for critical arrow, current chance: " + chance + "%");
