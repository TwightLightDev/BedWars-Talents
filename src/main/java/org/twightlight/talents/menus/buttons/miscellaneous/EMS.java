...
public class EMS {
    ...
    public EMS() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "TNTD") < 10) {
            ...
            lore.add("&7Khi mua 1 quả TNT có xác suất " + Utility.toDecimal(level*3.5) + "%.");
            lore.add("&7nhận được 1 quả bóng lửa.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "FBD") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            return ConversionUtil.createItem(Material.EMERALD, level, "",
                    0, "&ePhần thưởng" + roman,
                    lore,
                    enchanted);
        });
