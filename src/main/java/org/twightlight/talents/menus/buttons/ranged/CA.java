...
public class CA {
    ...
    public CA() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Ranged, "DLC") < 10) {
            ...
            lore.add("&7Mũi tên tăng " + Utility.toDecimal(level*1.5) +"% tỷ lệ chí mạng.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "DLC") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cNhân đôi X");
                req = false;
            }
