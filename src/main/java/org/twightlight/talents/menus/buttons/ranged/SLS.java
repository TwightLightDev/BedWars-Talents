...
public class SLS {
    ...
    public SLS() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Ranged, "MLS") < 10) {
            ...
            lore.add("&7Tăng tỷ lệ hút máu của mũi tên thêm " + Utility.toDecimal(level*0.5) + "%.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "MLS") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
