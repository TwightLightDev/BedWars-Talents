...
public class IHE {
    private int x = 0;
    private int y = 10;
    private TalentsCategory category = TalentsCategory.Melee;
    private TalentsCategory category1 = TalentsCategory.Protective;

    private String id = "IHE";
    ...
    public IHE() {
        Button button = new Button((e) -> {
            ...
            Talent<?> talent1 = Talents.getInstance().getTalentsManagerService().Talents.get(category1.getColumn()).get(id);
            List<Integer> costlist = talent.getCostList();
            if (database.getTalentLevel(p, TalentsCategory.Melee, "SLS") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Protective, "SRG") < 10) {
            ...
            if (e.isLeftClick()) {
                ...
                if (soulstones >= Utility.totalCost(costlist, level, level)) {
                    ...
                    database.upgradeTalents(1, talent1, id, p);
                    ...
                    if (!Utility.isInteger(s)) {
                        ...
                        if (level + increment > costlist.size()) {
                            ...
                            if (database.getSoulStones(p) < totalCost) {
                                ...
                                database.upgradeTalents(increment, talent1, id, p);
            ...
            lore.add("&7Tăng " + Utility.toDecimal(level) + "% khả năng hồi phục.");
            List<Integer> costlist = Talents.getInstance().getTalentsManagerService().Talents.get(category.getColumn()).get(id).getCostList();
            boolean enchanted = false;
            boolean req = true;
            boolean first = true;
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "SLS") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cLưỡi đao máu X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.WOOL, level, "",
                    ConversionUtil.getDataValue(DyeColor.LIME), "&eKotex thấm hút" + roman,
                    lore,
                    enchanted);
        });
