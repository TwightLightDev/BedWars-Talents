...
public class BLC {
    ...
    public BLC() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Protective, "BL") < 10) {
            ...
            lore.add("&7Tăng " + Utility.toDecimal(level) + "% tỷ lệ kích hoạt khiên.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "BL") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cKhiên huyền thoại X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.BEDROCK, level, "",
                    0, "&eChống đỡ" + roman,
                    lore,
                    enchanted);
        });
