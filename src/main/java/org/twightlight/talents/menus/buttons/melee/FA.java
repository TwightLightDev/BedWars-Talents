...
public class FA {
    ...
    public FA() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "MAD") < 10) {
            ...
            lore.add("&7Tấn công cận chiến gây thiêu đốt đối thủ");
            lore.add("&7với xác suất " + Utility.toDecimal(level*1.5) + "% trong " + Utility.toDecimal((double) (1 + Math.round(level / 5))) + " giây.");
            List<Integer> costlist = Talents.getInstance().getTalentsManagerService().Talents.get(category.getColumn()).get(id).getCostList();
            boolean enchanted = false;
            boolean req = true;
            boolean first = true;
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "MAD") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            return ConversionUtil.createItem(Material.BLAZE_POWDER, level, "",
                    0, "&eDo thái" + roman,
                    lore,
                    enchanted);
        });
