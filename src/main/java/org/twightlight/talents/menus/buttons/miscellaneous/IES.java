...
public class IES {
    ...
    public IES() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "IWD") < 10) {
            ...
            lore.add("&7Bắt đầu trận đấu nhận được " + Utility.toDecimal(level) + " đá kết thúc.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "IWD") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cGỗ khởi đầu X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.ENDER_STONE, level, "",
                    0, "&eĐá khởi đầu" + roman,
                    lore,
                    enchanted);
        });
