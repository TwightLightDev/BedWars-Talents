...
public class CD {
    ...
    public CD() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "CH") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Ranged, "CA") < 10) {
            ...
            lore.add("&7Tăng STCM thêm " + Utility.toDecimal(level*2.5) + "%.");
            List<Integer> costlist = Talents.getInstance().getTalentsManagerService().Talents.get(category.getColumn()).get(id).getCostList();
            boolean enchanted = false;
            boolean req = true;
            boolean first = true;
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "CH") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cChí mạng (cân chiến) X");
                req = false;
            }
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "CA") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                }
                lore.add("&cChí mạng (mũi tên) X");
                req = false;
            }
            if (req) {
                if (level >= costlist.size()) {
