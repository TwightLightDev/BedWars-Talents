...
public class SLD {
    ...
    public SLD() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Supportive, "WR") < 10) {
            ...
            lore.add("&7Khi kết liễu đối thủ, nhận được " + Utility.toDecimal((level * 0.1)) + "% TL kích hoạt khiên");
            lore.add("&7và " + Utility.toDecimal(level * 0.1) + "% TL giảm ST của khiên, cộng dồn tối đa 5 lần.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Supportive, "WR") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cChiến binh X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.OBSIDIAN, level, "",
                    0, "&eAnh hùng khiên" + roman,
                    lore,
                    enchanted);
        });
