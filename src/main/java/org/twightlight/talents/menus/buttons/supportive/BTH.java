...
public class BTH {
    ...
    public BTH() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "GGF") < 10) {
            ...
            lore.add("&7Khi kết liễu đối thủ, nhận được " + Utility.toDecimal(level) + " thỏi sắt và " + Utility.toDecimal(1 + Math.round(level/10)) + " thỏi vàng.");
            if (level <= 0) {
                lore.clear();
                lore.add("&7Khi kết liễu đối thủ, nhận được tài nguyên.");
            }
            ...
            return ConversionUtil.createItem(Material.GOLD_SPADE, level, "",
                    0, "&eThợ săn tiền thưởng" + roman,
                    lore,
                    enchanted);
        });
