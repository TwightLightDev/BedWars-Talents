    ...
    public AP() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "SLS") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Ranged, "SLS") < 10) {
            ...
            lore.add("&7Tăng " + Utility.toDecimal(level*1.2) + "% xuyên giáp.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "SLS") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                }
                lore.add("&cMũi tên máu X");
                req = false;
            }
            if (req) {
                if (level >= costlist.size()) {
