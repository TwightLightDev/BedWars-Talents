...
public class SA {
    ...
    public SA() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Ranged, "AAD") < 10) {
            ...
            lore.add("&7Mỗi khi bắn tên trúng đích, có xác suất");
            lore.add("&7" + Utility.toDecimal(level*2.5) + "% nhận được Tốc độ I. Hiệu ứng kéo dài");
            lore.add("&7" + Utility.toDecimal((double) (2 + Math.round(level / 4))) + " giây.");
            List<Integer> costlist = Talents.getInstance().getTalentsManagerService().Talents.get(category.getColumn()).get(id).getCostList();
            boolean enchanted = false;
            boolean req = true;
            boolean first = true;
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "AAD") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cMũi tên kim cương X");
                req = false;
            }
            if (req) {
                if (level >= costlist.size()) {
            ...
            return ConversionUtil.createItem(Material.LEATHER_BOOTS, level, "",
                    0, "&eBắn xong chạy" + roman,
                    lore,
                    enchanted);
        });
