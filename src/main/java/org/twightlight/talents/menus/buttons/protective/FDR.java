...
public class FDR {
    ...
    public FDR() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Protective, "MHP") < 10) {
            ...
            lore.add("&7Giảm " + Utility.toDecimal(level*2.5) + "% sát thương rơi nhận vào.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "MHP") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cMáu tối đa X");
                req = false;
            }
