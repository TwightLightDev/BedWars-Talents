...
public class IRSS {
    ...
    public IRSS() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Supportive, "BTH") < 10) {
            ...
            lore.add("&7Bắt đầu trận đấu nhận được " + Utility.toDecimal(level * 3) + " thỏi sắt và");
            lore.add("&7" + Utility.toDecimal(1 + Math.round(level/4) + 2 * Math.round(level/20)) + " thỏi vàng.");
            if (level <= 0) {
                lore.clear();
                lore.add("&7Bắt đầu trận đấu nhận một lượng tài nguyên.");
            }
            ...
            return ConversionUtil.createItem(Material.GOLD_PICKAXE, level, "",
                    0, "&eGói tài nguyên" + roman,
                    lore,
                    enchanted);
        });
