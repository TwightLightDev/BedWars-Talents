...
public class SM {
    ...
    public SM() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Ranged, "AKB") < 10) {
            ...
            lore.add("&7Mũi tên bắn trúng đối thủ có xác suất " + Utility.toDecimal(level*3.5) + "%.");
            lore.add("&7triệu hồi 1 bedbug trong " + Utility.toDecimal(10 + level) + " giây.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "AKB") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cTránh xa tao ra X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.SNOW_BALL, level, "",
                    0, "&eTriệu hồi sư" + roman,
                    lore,
                    enchanted);
        });
