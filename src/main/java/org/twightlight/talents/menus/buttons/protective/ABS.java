...
public class ABS {
    ...
    public ABS() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Supportive, "SHR") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Miscellaneous, "GGF") < 10) {
            ...
            lore.add("&7Nhận sát thương có " + Utility.toDecimal(level * 2) + "% tỷ lệ nhận được 2 tim vàng.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Supportive, "SHR") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "GGF") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            return ConversionUtil.createItem(Material.GOLDEN_APPLE, level, "",
                    0, "&eHấp thụ" + roman,
                    lore,
                    enchanted);
        });
