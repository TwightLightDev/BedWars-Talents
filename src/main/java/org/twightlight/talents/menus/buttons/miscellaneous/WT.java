...
public class WT {
    ...
    public WT() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Protective, "SRG") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Supportive, "BTH") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Protective, "ABS") < 10) {
            ...
            lore.add("&7Khi hồi sinh, nhận được " + Utility.toDecimal(level*2) + " len.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "ABS") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Supportive, "BTH") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cThợ săn tiền thưởng X");
                req = false;
            }

            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "SRG") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cTự phục hồi X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.SKULL_ITEM, level, "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNDRiMDM3OTRiOWIzZTNiNWQwN2UzYmU2OGI5NmFmODdkZjIxNWMzNzUyZTU0NzM2YzgwZjdkNTBiZDM0MzdhNCJ9fX0=",
                    3, "&eMộ len" + roman,
                    lore,
                    enchanted);
        });
