...
public class IWD {
    ...
    public IWD() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "IW") < 10) {
            ...
            lore.add("&7Bắt đầu trận đấu nhận được " + Utility.toDecimal(level) + " gỗ.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "IW") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cLen khởi đầu X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.WOOD, level, "",
                    0, "&eGỗ khởi đầu" + roman,
                    lore,
                    enchanted);
        });
