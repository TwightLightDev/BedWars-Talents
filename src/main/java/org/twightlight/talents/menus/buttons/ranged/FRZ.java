...
public class FRZ {
    ...
    public FRZ() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Ranged, "SA") < 10) {
            ...
            lore.add("&7" + Utility.toDecimal(level*1.5) + "% đóng băng địch trong 1 giây.");
            List<Integer> costlist = Talents.getInstance().getTalentsManagerService().Talents.get(category.getColumn()).get(id).getCostList();
            boolean enchanted = false;
            boolean req = true;
            boolean first = true;
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "SA") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cBắn xong chạy X");
                req = false;
            }
            if (req) {
                if (level >= costlist.size()) {
            ...
            return ConversionUtil.createItem(Material.PACKED_ICE, level, "",
                    0, "&eCông túa băng giá" + roman,
                    lore,
                    enchanted);
        });
