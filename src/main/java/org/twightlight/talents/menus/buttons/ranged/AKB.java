...
public class AKB {
    ...
    public AKB() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Ranged, "AP") < 10) {
            ...
            lore.add("&7Tăng khả năng đẩy lùi của mũi tên thêm " + Utility.toDecimal(level*2) + "%.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "AP") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            return ConversionUtil.createItem(Material.STICK, level, "",
                    0, "&eTránh xa tao ra" + roman,
                    lore,
                    enchanted);
        });
