...
public class DR {
    ...
    public DR() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Protective, "YH") < 10) {
            ...
            lore.add("&7Giảm " + Utility.toDecimal(level*1.25) + "% sát thương nhận vào.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "YH") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cTim vàng X");
                req = false;
            }
