...
public class BL {
    ...
    public BL() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "IRS") < 10) {
            ...
            lore.add("&7Nhận sát thương có " + Utility.toDecimal(level) + "% tỷ lệ kích hoạt khiên,");
            lore.add("&7giảm " + Utility.toDecimal(level * 2.5)+ "% ST nhận vào.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "IRS") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cHồi sinh nhanh X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.GLASS, level, "",
                    ConversionUtil.getDataValue(DyeColor.YELLOW), "&eKhiên thất truyền" + roman,
                    lore,
                    enchanted);
        });
