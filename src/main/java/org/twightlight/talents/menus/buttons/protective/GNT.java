...
public class GNT {
    ...
    public GNT() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "EST") < 10) {
            ...
            lore.add("&7Sau khi bắt đầu trận đấu, cứ mỗi " + Utility.toDecimal(3 * ((31-level) - Math.floor(level/20))) + " giây sẽ");
            lore.add("&7được tăng 1 máu tối đa. Tối đa 5 điểm.");
            if (level <= 0) {
                lore.clear();
                lore.add("&7Sau khi bắt đầu trận đấu, cứ mỗi một lúc sẽ");
                lore.add("&7được tăng 1 máu tối đa. Tối đa 5 điểm.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "EST") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            return ConversionUtil.createItem(Material.DIAMOND_CHESTPLATE, level, "",
                    0, "&eKhổng lồ hóa" + roman,
                    lore,
                    enchanted);
        });
