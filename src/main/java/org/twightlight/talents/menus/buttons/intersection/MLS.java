...
public class MLS {
    ...
    private TalentsCategory category1 = TalentsCategory.Ranged;

    private String id = "MLS";
    ...
    public MLS() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "MSL") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Ranged, "FRZ") < 10) {
            ...
            lore.add("&7Tăng " + Utility.toDecimal(level*1.5) + "% hút máu.");
            List<Integer> costlist = Talents.getInstance().getTalentsManagerService().Talents.get(category.getColumn()).get(id).getCostList();
            boolean enchanted = false;
            boolean req = true;
            boolean first = true;
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "MSL") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cBăng lạnh X");
                req = false;
            }
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "FRZ") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                }
                lore.add("&cCông túa băng giá X");
                req = false;
            }
            if (req) {
                if (level >= costlist.size()) {
            ...
            return ConversionUtil.createItem(Material.SPIDER_EYE, level, "",
                    0, "&eMa cà rồng" + roman,
                    lore,
                    enchanted);
        });
