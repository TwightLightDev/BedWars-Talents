...
public class EST {
    ...
    public EST() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "WDT") < 10) {
            ...
            lore.add("&7Khi hồi sinh, nhận được " + Utility.toDecimal(level) + " đá kêt thúc.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "WDT") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
