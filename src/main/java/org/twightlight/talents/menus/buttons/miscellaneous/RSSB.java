...
public class RSSB {
    ...
    public RSSB() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "AGF") < 10) {
            ...
            lore.add("&7Mỗi khi nhặt tài nguyên, có "+ Utility.toDecimal(level * 0.75)+ "% được thưởng thêm");
            lore.add("&71 vật phẩm cùng loại.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "AGF") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cMạnh thường quân X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.CHEST, level, "",
                    0, "&eHãy trao cho anh" + roman,
                    lore,
                    enchanted);
        });
