...
public class ASA {
    ...
    public ASA() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Supportive, "SLD") < 10) {
            ...
            lore.add("&7Khi kết liễu đối thủ, nhận được " + Utility.toDecimal((level * 0.1)) + "% mọi TLCM");
            lore.add("&7và " + Utility.toDecimal(level * 0.25) + "% STCM, cộng dồn tối đa 5 lần.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Supportive, "SLD") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cAnh hùng khiên X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.BONE, level, "",
                    0, "&eSad thủ" + roman,
                    lore,
                    enchanted);
        });
