...
public class APR {
    ...
    public APR() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "RSSB") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Protective, "BLC") < 10) {
            ...
            lore.add("&7Khi bắt đầu trận đấu, nhận được " + Utility.toDecimal(1 + Math.round(level / 5)) + " vật phẩm linh tinh");
            lore.add("&7ngẫu nhiên, có xác suất " + Utility.toDecimal(level * 1.5) + "% nhận được vật phẩm có giá trị cao.");
            if (level <= 0) {
                lore.clear();
                lore.add("&7Khi bắt đầu trận đấu, nhận được một số vật phẩm linh tinh");
                lore.add("&7 ngẫu nhiên, có xác suất nhận được vật phẩm có giá trị cao.");
            }
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "RSSB") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cHãy trao cho anh X");
                req = false;
            }
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "BLC") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cChống đỡ X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.BEACON, level, "",
                    0, "&eJack 97" + roman,
                    lore,
                    enchanted);
        });
