...
public class CH {
    ...
    public CH() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "AMAD") < 10) {
            ...
            lore.add("&7Cận chiến tăng " + Utility.toDecimal(level) +"% tỷ lệ chí mạng.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "AMAD") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cMa gaeming booyah X");
                req = false;
            }
