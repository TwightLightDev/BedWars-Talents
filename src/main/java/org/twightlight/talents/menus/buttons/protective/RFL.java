package org.twightlight.talents.menus.buttons.protective;
...
public class RFL {
    private int x = 1;
    private int y = 22;
    private TalentsCategory category = TalentsCategory.Protective;
    private String id = "RFL";
    ...
    public RFL() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Protective, "THR") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Protective, "BLC") < 15) {
            ...
            lore.add("&7Mỗi khi nhận sát thương, có "+ Utility.toDecimal(level * 1.25)+ "% phản lại một đòn");
            lore.add("&7cận chiến với hệ số sát thương gốc là 3 và giảm");
            lore.add("&725% sát thương phải chịu từ đòn tấn công đó.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "THR") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "BLC") < 15) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cChống đỡ XV");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.RAW_FISH, level, "",
                    3, "&eCá nóc" + roman,
                    lore,
                    enchanted);
        });
