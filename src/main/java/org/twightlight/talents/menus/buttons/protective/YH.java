...
public class YH {
    ...
    public YH() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Protective, "FDR") < 10) {
            ...
            lore.add("&7Nhận " + Utility.toDecimal(level) + " tim vàng khi bắt đầu trận đấu.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "FDR") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cLông cặc X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.GOLD_CHESTPLATE, level, "",
                    0, "&eTim vàng" + roman,
                    lore,
                    enchanted);
        });
