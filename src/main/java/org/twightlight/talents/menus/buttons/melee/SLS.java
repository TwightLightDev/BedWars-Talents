...
public class SLS {
    ...
    public SLS() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "MLS") < 10) {
            ...
            lore.add("&7Tăng tỷ lệ hút máu cận chiến thêm " + Utility.toDecimal(level*0.5) + "%.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "MLS") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cMa cà rồng X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.IRON_AXE, level, "",
                    0, "&eLưỡi đao máu" + roman,
                    lore,
                    enchanted);
        });
