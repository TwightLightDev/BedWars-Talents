...
public class ARS {
    ...
    public ARS() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Supportive, "ASA") < 10 ||
            database.getTalentLevel(p, TalentsCategory.Miscellaneous, "RSSB") < 10) {
            ...
            lore.add("&7Khi kết liễu đối thủ, nhận được " + Utility.toDecimal((level * 0.025)) + " công");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Supportive, "ASA") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            return ConversionUtil.createItem(Material.GOLD_AXE, level, "",
                    0, "&eThần chiến tranh" + roman,
                    lore,
                    enchanted);
        });
