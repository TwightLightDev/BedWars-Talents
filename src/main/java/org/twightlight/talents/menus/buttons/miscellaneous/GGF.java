...
public class GGF {
    ...
    public GGF() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "IES") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Miscellaneous, "EMS") < 10) {
            ...
            lore.add("&7Sau khi bắt đầu trận đấu, cứ mỗi " + Utility.toDecimal((31-level) - Math.floor(level/20)) + " giây sẽ");
            lore.add("&7nhận được 1 thỏi vàng.");
            if (level <= 0) {
                lore.clear();
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "EMS") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cPhần thưởng X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.GOLD_INGOT, level, "",
                    0, "&eMidas" + roman,
                    lore,
                    enchanted);
        });
