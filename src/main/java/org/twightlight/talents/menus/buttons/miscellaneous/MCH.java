...
public class MCH {
    ...
    public MCH() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Miscellaneous, "WT") < 10) {
            ...
            lore.add("&7Mỗi khi mua khối len, được thưởng thêm " + Utility.toDecimal(level) + " khối len.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "WT") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            return ConversionUtil.createItem(Material.IRON_INGOT, level, "",
                    0, "&eKhuyễn mãi" + roman,
                    lore,
                    enchanted);
        });
