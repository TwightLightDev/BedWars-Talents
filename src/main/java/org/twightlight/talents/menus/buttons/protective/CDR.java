...
public class CDR {
    ...
    public CDR() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Protective, "GNT") < 10) {
            ...
            lore.add("&7Giảm " + Utility.toDecimal(level * 2) + "% ST chí mạng nhận vào.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "GNT") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cKhổng lồ hóa X");
                req = false;
            }
