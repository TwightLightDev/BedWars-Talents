...
public class THR {
    ...
    public THR() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Protective, "BLC") < 10 ||
                    database.getTalentLevel(p, TalentsCategory.Melee, "CH") < 10) {
            ...
            lore.add("&7Giảm " + Utility.toDecimal(level * 2.5) + "% khả năng hút máu của kẻ tấn công.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "CH") < 10) {
                ...
                lore.add("&cChí mạng (cận chiến) X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.SLIME_BALL, level, "",
                    0, "&eGayyyy" + roman,
                    lore,
                    enchanted);
        });
