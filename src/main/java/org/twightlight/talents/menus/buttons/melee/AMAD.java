...
public class AMAD {
    ...
    public AMAD() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "AMA") < 10) {
            ...
            lore.add("&7Tỷ số ST gây ra dựa trên sát thương đòn đánh gốc");
            lore.add("&7của đòn đánh phụ được tăng thêm " + Utility.toDecimal((level * 0.5))  + "%.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "AMA") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cMa gaeming X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.ANVIL, level, "",
                    0, "&eMa gaeming booyah" + roman,
                    lore,
                    enchanted);
        });
