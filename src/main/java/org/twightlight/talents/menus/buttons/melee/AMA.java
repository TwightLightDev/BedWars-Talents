...
public class AMA {
    ...
    public AMA() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "MAS") < 10) {
            ...
            lore.add("&7Đòn đánh thường có xác suất " + Utility.toDecimal(level*2.5) + "% gây ra thêm");
            lore.add("&71 đòn đánh phụ. Đòn đánh này có hệ số ST bằng " + Utility.toDecimal((10 + (level * 0.5))) + "%");
            lore.add("&7ST đòn đánh gốc.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "MAS") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cTấn công nhanh X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.GOLD_SWORD, level, "",
                    0, "&eMa gaeming" + roman,
                    lore,
                    enchanted);
        });
