...
public class DLC {
    ...
    public DLC() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Ranged, "SM") < 10) {
            ...
            lore.add("&7Khi triệu hồi bedbug bởi tài năng &7&lTriệu hồi sư, &7có");
            lore.add("&7xác suất " + Utility.toDecimal(level*4.25) + "% triệu hồi ra thêm 1 bedbug.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "SM") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cTriệu hồi sư X");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.DISPENSER, level, "",
                    0, "&eNhân đôi" + roman,
                    lore,
                    enchanted);
        });
