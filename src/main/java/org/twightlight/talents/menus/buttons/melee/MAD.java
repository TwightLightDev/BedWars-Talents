...
public class MAD {
    ...
    public MAD() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "IMD") < 10) {
            ...
            lore.add("&7Tăng ST cận chiến thêm " + Utility.toDecimal(level*0.05) + ".");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "IMD") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            return ConversionUtil.createItem(Material.IRON_SWORD, level, "",
                    0, "&eSắc bén" + roman,
                    lore,
                    enchanted);
        });
