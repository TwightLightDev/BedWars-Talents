...
public class MSL {
    ...
    public MSL() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "FA") < 10) {
            ...
            lore.add("&7Tấn công cận chiến gây hiệu ứng làm chậm I");
            ...
            boolean req = true;
            boolean first = true;
            if (Talents.getInstance().getDatabase().
                    getTalentLevel(player, TalentsCategory.Melee, "FA") < 10) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            return ConversionUtil.createItem(Material.SNOW_BLOCK, level, "",
                    0, "&eBăng lạnh" + roman,
                    lore,
                    enchanted);
        });
