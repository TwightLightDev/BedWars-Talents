package org.twightlight.talents.menus.buttons.special;
...
public class KOBW {
    private int x = 0;
    private int y = 24;
    private TalentsCategory category = TalentsCategory.Special;
    private String id = "KOBW";
    ...
    public KOBW() {
        Button button = new Button((e) -> {
            ...
            if (database.getTalentLevel(p, TalentsCategory.Melee, "TH") < 15 ||
                    database.getTalentLevel(p, TalentsCategory.Ranged, "TWIN") < 15 ||
                    database.getTalentLevel(p, TalentsCategory.Protective, "RFL") < 15 ||
                    database.getTalentLevel(p, TalentsCategory.Miscellaneous, "APR") < 15 ||
                    database.getTalentLevel(p, TalentsCategory.Supportive, "ARS") < 15) {
            ...
            lore.add("&7Mỗi khi gây sát thương:");
            lore.add("&7 + Có xác suất " + Utility.toDecimal(level * 2.5) + "% tăng " + Utility.toDecimal(level) + "% sát thương.");
            lore.add("&7Mỗi khi nhận sát thương:");
            lore.add("&7 + Có xác suất " + Utility.toDecimal(level * 2.5) + "% giảm " + Utility.toDecimal(level * 0.8) + "% sát thương.");
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Melee, "TH") < 15) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cThần zú XV");
                req = false;
            }
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Ranged, "TWIN") < 15) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cSong tiễn XV");
                req = false;
            }
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Protective, "RFL") < 15) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
            ...
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Miscellaneous, "APR") < 15) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cJack 97 XV");
                req = false;
            }
            if (Talents.getInstance().getDatabase().getTalentLevel(player, TalentsCategory.Supportive, "ARS") < 15) {
                if (first) {
                    lore.add("");
                    lore.add("&cYêu cầu:");
                    first = false;
                }
                lore.add("&cThần chiến tranh XV");
                req = false;
            }
            ...
            return ConversionUtil.createItem(Material.BED, level, "",
                    0, "&eVua giường chiếu" + roman,
                    lore,
                    enchanted);
        });
