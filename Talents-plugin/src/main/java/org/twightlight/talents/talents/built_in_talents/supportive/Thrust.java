...
public class Thrust extends Talent {

    public Thrust(String talentId, List<Integer> costList) {
    ...
    public void onPlayerKill(PlayerKillEvent e) {
        ...
        if (kills <= 5) {
            ...
            statsMap.getStatContainer(BaseStats.PENETRATION_RATIO.name()).add(add);
            statsMap.getStatContainer(BaseStats.ENCHANTMENT_BREAK_RATIO.name()).add(add);
        }
